...
async def list_drafts(
    ...
    """Lists all AI-generated drafts."""
    return repo.list_drafts(limit=limit)
...
async def get_draft_for_email(email_id: str, current_user: dict = Depends(get_current_user)):
    """Drafts are generated automatically during email sync/ingest - this reads the latest one for an email."""
...
async def approve_draft(draft_id: str, current_user: dict = Depends(get_current_user)):
    """DRAFT-03 - HR clicks approve, the draft is sent as a real reply (or trial dispatch in demo mode)."""
